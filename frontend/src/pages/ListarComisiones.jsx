...
export default function ListarComisiones() {
  ...
  const generalFiltered = comisiones.filter((u) => {
  ...
  const handleEdit = (item) => navigate(`/comisiones/update/${item.IDComision}`);

  const handleDelete = async (item) => {
    const confirmed = await AlertMessage.confirm("Eliminar comisión", `¿Eliminar la comisión ${item.nombre}?`);
    if (!confirmed) return;

    try {
      const res = await fetch(import.meta.env.VITE_PATH + `/comisiones/${item.IDComision}`, { method: "DELETE", credentials: 'include' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Error al eliminar comisión");
      setComisiones((prev) => prev.filter((c) => c.IDComision !== item.IDComision));
      AlertMessage.success("Eliminado", "La comisión fue eliminada correctamente.");
    } catch (err) {
      console.error(err);
      AlertMessage.error("Error", err.message || "No se pudo eliminar la comisión.");
    }
  };
