...
export default function EditarInstitucion() {
  ...
  useEffect(() => {
    fetch(import.meta.env.VITE_PATH + `/instituciones/${id}`, {
      credentials: "include",
    })
      .then(res => res.json())
      .then(data => setNombre(data.NombreInstitucion));
  }, [id]);

  const handleSubmit = async (e) => {
    e.preventDefault();

    const res = await fetch(import.meta.env.VITE_PATH + `/instituciones/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ NombreInstitucion: nombre }),
    });
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                type="button"
                onClick={() => navigate("/instituciones/listar")}
