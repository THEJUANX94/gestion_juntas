  ...
  const downloadReport = async (reportKey, format, extra = {}) => {
    try {
      ...
      const res = await fetch(
        `${VITE_PATH}/api/juntas/reports/${backendKey}/export?${params.toString()}`,
        { credentials: "include" }
      );
      if (!res.ok) throw new Error(`Error ${res.status}`);
              ...
              {selectedReport === "province" && (
                <div>
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-[#009E76] to-[#64AF59] px-5 py-2 text-white"
                  >
                    <Download size={16} />
                    Cargar provincias
                  </button>
                    ...
                    <>
                      ...
                      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                        {(provinceData.labels || []).map((prov, i) => (
                          <div key={prov} className="rounded-lg border p-4">
                            ...
                            <p className="mt-2 text-2xl font-bold text-[#009E76]">
                              {provinceData.series?.[i] || 0}
                            </p>
                            <DownloadButtons
                              compact
                              onDownload={(format) =>
                                downloadReport("province", format, { provincias: [prov] })
                              }
                            />
                          </div>
                        ))}
                ...
                <div>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded bg-gray-200 px-3 py-2 text-sm"
                    >
                      <RotateCcw size={14} />
                      Limpiar
                    </button>
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded bg-gradient-to-r from-[#009E76] to-[#64AF59] px-4 py-2 text-sm text-white disabled:opacity-50"
                    >
                      <Download size={14} />
                      Ver juntas
                    </button>
