...
export default function CrearOrganismo() {
	...
	const handleSubmit = async (e) => {
		...
		setLoading(true);
		try {
			const res = await fetch(import.meta.env.VITE_PATH + "/tipojunta/creartipojunta", {
				method: "POST",
				credentials: 'include',
			...
			if (!res.ok) throw new Error(data.message || "Error al crear organismo");

			AlertMessage.success("Creado", "Organismo creado correctamente");
			navigate("/organismos/listar");
		} catch (err) {
			console.error(err);
			AlertMessage.error("Error", err.message || "No se pudo crear el organismo");
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<div className="flex flex-col h-full">
			<h1 className="text-2xl mb-6" style={{ color: "var(--color-text-color-page)" }}>Crear Organismo</h1>
