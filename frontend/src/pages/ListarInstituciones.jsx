...
export default function ListarInstituciones() {
	...
	useEffect(() => {
		const fetchInstituciones = async () => {
			try {
				const res = await fetch(import.meta.env.VITE_PATH + "/instituciones", { credentials: 'include', method: "GET" });
				const data = await res.json();
				const transformados = data.map((c) => ({
					IDInstitucion: c.IDInstitucion,
					nombre: c.NombreInstitucion,
				}));
				setInstituciones(transformados);
			} catch (error) {
	...
	const generalFiltered = instituciones.filter((u) => {
	...
	const handleEdit = (item) => navigate(`/instituciones/update/${item.IDInstitucion}`);

	const handleDelete = async (item) => {
		const confirmed = await AlertMessage.confirm("Eliminar institución", `¿Eliminar la institución ${item.nombre}?`);
		if (!confirmed) return;
		try {
			const res = await fetch(import.meta.env.VITE_PATH + `/instituciones/${item.IDInstitucion}`, { method: "DELETE", credentials: 'include' });
			if (!res.ok) throw new Error("Error al eliminar institución");
			setInstituciones((prev) => prev.filter((c) => c.IDInstitucion !== item.IDInstitucion));
			AlertMessage.success("Eliminado", "La institución fue eliminada correctamente.");
		} catch (err) {
			console.error(err);
			AlertMessage.error("Error", "No se pudo eliminar la institución.");
		}
	};
